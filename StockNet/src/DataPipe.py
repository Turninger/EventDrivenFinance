...
import random
from ConfigLoader import path_parser, config_model, dates, stock_symbols, vocab, vocab_size
import  time
class DataPipe:
                ...
                with open(msg_fp, 'r') as tweet_f:
                    for line in tweet_f:
                        ...
                        if msg_id == self.max_n_msgs:
                            break
                corpus = [d, word_mat[:msg_id], ss_index_vec[:msg_id], n_word_vec[:msg_id], msg_id] # need check on why :msg_id
    ...
    def batch_gen(self, phase):
        ...
        while True:
            ...
            ss_index_batch = np.zeros([batch_size, self.max_n_days, self.max_n_msgs], dtype=np.int32) # what's ss_index for?
