...
def path2tensor(row, max_news_cnt = 4, embedded_size = 300):
    filePath = dataPath + '/stockNewsVec'
    tensor = []
...
for s in ['cv', 'test', 'train']:
    dataloaderFile = dataPath + f'/dataloader/{s}_data_full_1.csv'
    ...
    np.save(dataPath + f'/dataloader/{s}_bert_np_full_1', ts, allow_pickle = False)
    del ts